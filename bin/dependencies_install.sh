...
if ! command_exists nvm; then
  ...
  # Add automatic nvm switching based on .nvmrc files
  info "Adding automatic nvm switching to shell config..."
  SHELL_RC="$HOME/.bashrc"
  if [ -f "$HOME/.zshrc" ]; then
    SHELL_RC="$HOME/.zshrc"
  fi
  
  # Check if the function is already in the config file
  if ! grep -q "cdnvm()" "$SHELL_RC"; then
    cat << 'EOF' >> "$SHELL_RC"

# Automatically switch Node.js versions when directory contains .nvmrc file
cdnvm() {
    command cd "$@" || return $?
...
EOF
    ...
    success "Added automatic nvm switching to $SHELL_RC"
  else
    info "Automatic nvm switching is already configured"
  fi
